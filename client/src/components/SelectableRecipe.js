...
function SelectableRecipe({ recipe, selected, onClick }) {
    ...
    const containerClasses = (selected) ? `${styles.recipe} ${styles.selected}` : styles.recipe;

    return (
        <div className={containerClasses} onClick={() => {onClick(recipe._id)}}>
            <h4>{recipe.name}</h4>
